...
const normalizePayload = (payload = {}) => {
  ...
  normalized.description = parseMaybeJson(payload.description) || payload.description;
  normalized.publisher = payload.publisher ?? payload.publish ?? payload.publisherId;
...
const syncBookRelations = async (productId, previousProduct = {}, nextProduct = {}) => {
  ...
  if (
    ...
    (previousCategoryId !== nextCategoryId || previousSubCategoryId !== nextSubCategoryId)
  ) {
    updates.push(
      Category.updateOne(
  ...
  if (previousAuthorId && previousAuthorId !== nextAuthorId) {
    updates.push(Author.updateOne({ _id: previousAuthorId }, { $pull: { books: productId } }));
  }

  if (nextAuthorId) {
    updates.push(Author.updateOne({ _id: nextAuthorId }, { $addToSet: { books: productId } }));
  }

  if (previousPublisherId && previousPublisherId !== nextPublisherId) {
    updates.push(Publisher.updateOne({ _id: previousPublisherId }, { $pull: { books: productId } }));
  }

  if (nextPublisherId) {
    updates.push(Publisher.updateOne({ _id: nextPublisherId }, { $addToSet: { books: productId } }));
  }
...
const getAllProducts = async (req, res, next) => {
  try {
    const { search, page = 1, limit = 10, category, author, publisher, publish, subCategoryId, subgenreId, subgenre } = req.query;
    let filter = {};
    ...
    const productsWithDetails = products.map(product => ({
      ...product.toObject(),
      categoryName: product.category?.title?.uz || product.category?.name?.uz || 'Noma\'lum',
...
const createProduct = async (req, res, next) => {
  try {
    ...
    const categoryValidation = await validateCategorySubgenre(payload.category, subCategoryId);
    if (categoryValidation.error) {
      return apiResponse(res, 400, false, categoryValidation.error);
    }

    const publisherValidation = await validatePublisher(resolvePublisherId(payload));
    ...
    if (Number.isNaN(priceNum)) {
      return apiResponse(res, 400, false, "price noto‘g‘ri (number bo‘lishi kerak)");
    }
    if (Number.isNaN(discountNum)) {
      return apiResponse(res, 400, false, "discountPrice noto‘g‘ri (number bo‘lishi kerak)");
    }
    if (Number.isNaN(stockNum)) {
      return apiResponse(res, 400, false, "stock noto‘g‘ri (number bo‘lishi kerak)");
    }
...
const updateProduct = async (req, res, next) => {
    try {
        ...
        const hasSubCategoryField = Object.prototype.hasOwnProperty.call(payload, 'subCategoryId')
        ...
        const categoryValidation = await validateCategorySubgenre(nextCategoryId, nextSubCategoryId);
        if (categoryValidation.error) {
            return apiResponse(res, 400, false, categoryValidation.error);
        }

        const hasPublisherField = Object.prototype.hasOwnProperty.call(req.body, 'publisher')
        ...
        if (price || discountPrice) {
            ...
            updateData.isDiscount = currentDiscount > 0 && currentDiscount < currentPrice;
        }
        ...
        apiResponse(res, 200, true, "Ma'lumotlar muvaffaqiyatli yangilandi", updatedProduct);
